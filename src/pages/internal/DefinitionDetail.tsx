...
export default function DefinitionDetail() {
  ...
  const startEdit = () => {
    setEditForm({
      name: def.name, category: def.category, manufacturer: def.manufacturer,
      model: def.model, description: def.description,
      expectedLifespan: def.expectedLifespan, msrp: def.msrp,
    })
  ...
  const cancelEdit = () => { setIsEditing(false); setEditForm({}); setEditSpecs([]) }
  ...
  const saveEdit = () => {
    ...
    setIsEditing(false); setEditForm({}); setEditSpecs([])
  }
  ...
  const doPublish = () => {
    ...
    setModal(null)
    if (activeVersion) {
      addToast({ type: 'success', title: 'Definition Published', message: `v${def.version} is now Active. v${activeVersion.version} has been set to Inactive.` })
    } else {
  ...
  const stateBadge = () => {
    if (def.state === 'Published' && def.versionState === 'Active')
      return <><span className="state-badge published">● Published</span><span className="state-badge active" style={{ marginLeft: 6 }}>◉ Active</span></>
    if (def.state === 'Published' && def.versionState === 'Inactive')
      return <><span className="state-badge published">● Published</span><span className="state-badge inactive" style={{ marginLeft: 6 }}>○ Inactive</span></>
    ...
    <>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{def.manufacturer} — {def.model} · v{def.version} · {def.category}</p>
        </div>
        <div className="page-actions" style={{ alignItems: 'center' }}>
          ...
          {def.state === 'Draft' && !isEditing && (
            <>
              <button className="btn btn-secondary" onClick={startEdit} style={{ marginLeft: 10 }}>Edit Draft</button>
              <button className="btn btn-primary"   onClick={tryPublish} style={{ marginLeft: 6 }}>Publish</button>
            </>
          )}
          {def.state === 'Draft' && isEditing && (
            <>
              <button className="btn btn-secondary" onClick={cancelEdit} style={{ marginLeft: 10 }}>Cancel</button>
              <button className="btn btn-primary"   onClick={saveEdit}   style={{ marginLeft: 6 }}>Save Changes</button>
            </>
          )}
          ...
          {def.state === 'Published' && def.versionState === 'Active' && (
            <>
              <button className="btn btn-secondary" onClick={() => setModal('confirm-new-version')} style={{ marginLeft: 10 }}>Create New Version</button>
              <button className="btn btn-secondary" onClick={() => setModal('confirm-archive')}     style={{ marginLeft: 6 }}>Archive</button>
            </>
          )}

          {/* Published + Inactive */}
          {def.state === 'Published' && def.versionState === 'Inactive' && (
            <button className="btn btn-secondary" onClick={() => setModal('confirm-archive')} style={{ marginLeft: 10 }}>Archive</button>
          )}

          {/* Archived */}
          {def.state === 'Archived' && (
            <button className="btn btn-primary" onClick={() => setModal('confirm-restore')} style={{ marginLeft: 10 }}>Restore to Catalog</button>
          )}
        </div>
      </div>
      ...
      {/* ── State notes (Draft + Archived only) ──────────────────────────────── */}
      {def.state === 'Draft' && (
        <div className="info-box warning">
          <span>✎</span>
          <span>
            {isEditing
              ? <>Edit mode — make your changes below and click <strong>Save Changes</strong>.</>
              : <>This draft is pending review and not yet visible in the customer catalog. Edit or publish when ready.</>}
          </span>
        </div>
      )}
      {def.state === 'Archived' && (
        <div className="info-box" style={{ background: 'var(--theme-color-ghost-selected)', borderColor: 'var(--theme-color-soft-bdr)' }}>
          <span>◎</span>
          <span>Archived — removed from the customer catalog. Existing asset links are preserved. Use <strong>Restore to Catalog</strong> to make it active again.</span>
        </div>
      )}
          ...
          {familyVersions.length > 1 && (
            <div className="card">
              ...
              <p style={{ fontSize: 12, color: 'var(--theme-color-soft-text)', marginBottom: 12 }}>
                All versions of the <strong>{def.name}</strong> definition family.
              </p>
              <table className="data-table" style={{ marginTop: 0 }}>
                <thead>
                  <tr>
              ...
              <div className="info-box info">
                ...
                <div>
                  ...
                  <p style={{ margin: '6px 0 0', fontSize: 12 }}>
                    ...
                    {activeVersion && <> v{activeVersion.version} (currently Active) will automatically be set to <strong>Inactive</strong>.</>}
                  </p>
                </div>
              </div>
            ...
            <div className="modal-body">
              <div className="info-box warning">
                ...
                <div>
                  ...
                  <p style={{ margin: '6px 0 0', fontSize: 12 }}>
                    This definition will be removed from the customer catalog. <strong>Existing commissioned asset links are preserved</strong>.
              ...
              {linkedAssetCount > 0 && (
                <div style={{ marginTop: 12, padding: '10px 14px', background: 'var(--theme-color-ghost-selected)', borderRadius: 6, fontSize: 13 }}>
                  <strong>{linkedAssetCount}</strong> asset{linkedAssetCount !== 1 ? 's are' : ' is'} linked to this definition and will retain their link after archiving.
                </div>
              )}
            </div>
            <div className="modal-footer">
